...
class es
{
	...
	/*
		1. First Sorting Logic
		foreach($final_array as $key=>$val){
			$sort[$key] = $val['_source']['udate'];
		}
		array_multisort($sort, SORT_ASC, $final_array);
		
		2. Second Sorting Logic
		usort($final_array, function($a, $b) {
			//return $a['str_time'] - $b['str_time']; // Accending Order
			//return $b['str_time'] - $a['str_time']; // Decending Order
			//return strtotime($b['_source']['udate']) - strtotime($a['_source']['udate']);
		});
	*/
}